...
<script>
let p = document.getElementById("firstP");  // 문서 객체 안에 아이디를 찾는 메소드, 속성값 id로 객체를 찾는다.
document.write("객체의 id는?" + p.id);
document.write("<br>");
document.write("객체의 태그는?" + p.tagName);
document.write("<br>");
document.write("객체가 담고있는 내용은?" + p.innerHTML);
document.write("<br>");
document.write("객체의 스타일?" + p.style.color);
document.write("<br>");
...
function textChange() {
	...
	myP.innerHTML = "무궁화 꽃이 피었습니다";
	
	return;
}

function fruit() {
	//let sp = document.getElementsByTagName("span"); // 여러개의 span 태그 컬렉션 집합
...
function winOpen() {
	...
	window.open("<%=request.getContextPath()%>/member/memberJoin.jsp","winName","width=300","height=300");
