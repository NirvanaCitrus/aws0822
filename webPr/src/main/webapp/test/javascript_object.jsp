...
<body>
	...
	// 2. DOM객체 (태그로 표현함)
	// 3. BOM객체 (브라우저를 컨트롤 할수 있게 만들어진 객체)
	
	// 1. 코어객체
	// 1-1 배열객체
	// []표현할수 있고 Array 객체로 할수도 있다.
	// 배열객체 생성
	...
	let intRoot = arrName.length;
	document.write("생성된 총 방의 갯수?" + intRoot+ "<br>");
	...
	let sum = 0;
	for (let i=0;i<aveScore.length;i++) {
		sum += aveScore[i];
	}
	
	let average = sum / aveScore.length;
	
	document.write("총합은?" + sum + "<br>");
	document.write("평균은?" + average + "<br>");
	
	// 1. 배열선언 세팅
	// 2. 반복문으로 합산
	// 3. 배열길이로 나누기
	
	document.write("자바스크립트 배열값에는 모든 타입의 값을 넣을수 있다");
	
	let any = new Array(5); // 5개의 방을 만듬
	...
	e = "";
	// c는 박,김,황 들어있음 역순으로
	e = c.slice(1,3);  // 1번 다음자리부터 3번자리까지
	document.write("e값은?" +e+ "<br>");
